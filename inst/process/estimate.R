...
estimateRandomEffectModel <- function(mildew, connectivity.scale, fixed.effects, mesh.params, tag="", type) {
  ...
  x <- mildew$setupModel(type=type, fixed.effects=fixed.effects, scale.covariates=FALSE, mesh.params=mesh.params, exclude.covariates=c("number_coinf","number_genotyped","Year","Longitude","Latitude","y"))
  mildew$plotMesh()
...
spa_INLA<-estimateRandomEffectModel(coin, connectivity.scale=coin.connectivity.scale, fixed.effects=coin.fixed.effects,
